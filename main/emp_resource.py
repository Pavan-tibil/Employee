...
class Employee(Resource):
    ...
    def put(self,emp_id):
        ...
        employee = Employee_Model.query.filter_by(emp_id=emp_id).first()
        if employee:
            employee.emp_name=args["name"] if args["name"] else employee.emp_name
            employee.emp_email=args["email"] if args["email"] else employee.emp_email
            employee.emp_age=args["age"] if args["age"] else employee.emp_age
            employee.emp_salary=args["salary"] if args["salary"] else employee.emp_salary
            db.session.add(employee)
            db.session.commit()
            return employee
        else:
